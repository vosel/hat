...
struct ParsedCsvRow
{
	...
	static auto parseDataRowString(std::string const & rowString); static auto parseDataRowString(std::string const & rowString); // TODO: add environments count parameter (make sure that the length of m_customColumns is consistent (not bigger and not smaller then expected))
};
